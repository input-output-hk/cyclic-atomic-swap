...
sequenceDiagram
    ...
    Note over A,C: "Verify commitments, compute leader,  elect A"
    ...
    Note over A: "A state = AwaitingSecrets"
    Note over B,C: "B, C states = AwaitingLeaderSpend: immediately broadcast secrets"
...
sequenceDiagram
    ...
    Note over B: ChainPollTarget::RefundWindow fires (W_B earliest)
    Chain-->>B: slot/block >= W_B
    B->>Chain: refund?: B reclaims (B?C locked funds)

    Note over C: ChainPollTarget::RefundWindow fires (W_C)
